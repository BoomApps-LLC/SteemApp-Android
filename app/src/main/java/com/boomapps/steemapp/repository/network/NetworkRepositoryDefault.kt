...
class NetworkRepositoryDefault : NetworkRepository {
    ...
    override fun postStory(
            ...
            tags: Array<String>,
            postingKey: String,
            rewardsPercent: Short,
            upvote: Boolean,
            callback: NetworkRepository.OnRequestFinishCallback<Any?>
    ) {
    ...
    override fun postStory(
        ...
        Observable.fromCallable {
            return@fromCallable RepositoryProvider.getSteemRepository()
                    .post(title, content, tags, postingKey, rewardsPercent, upvote, permlink)
        }
    ...
    override fun uploadNewPhoto(
            uri: Uri,
            callback: NetworkRepository.OnRequestFinishCallback<URL?>
    ) {
        var result: URL? = null
        Observable.fromCallable {
            return@fromCallable RepositoryProvider.getSteemRepository()
    ...
    override fun loadExtendedUserProfile(
            nick: String,
            callback: NetworkRepository.OnRequestFinishCallback<ProfileResponse?>
    ) {
    ...
    override fun loadFullStartData(
            nick: String,
            callback: NetworkRepository.OnRequestFinishCallback<Any?>
    ) {
        val coinmarketcapToUsdFloawable: Observable<Array<CoinmarketcapCurrency>> = getObservableForCurrency(
                "steem"
        )//.onErrorResumeNext { s: Subscriber<in Array<CoinmarketcapCurrency>>? -> Log.d("NetworkRepoDef", "steemToUsd error loading") }
        val coinmarketcapDollarToUsdFlowable: Observable<Array<CoinmarketcapCurrency>> = getObservableForCurrency(
                "steem-dollars"
        )//.onErrorResumeNext { s: Subscriber<in Array<CoinmarketcapCurrency>>? -> Log.d("NetworkRepoDef", "steemDollarToUsd error loading") }
        val balanceVestFlowable: Observable<Array<Double>> =
                getBalanceVestsObservable()//.onErrorResumeNext { s: Subscriber<in Array<Double>>? -> Log.d("NetworkRepoDef", "balanceVests error loading") }
        val exUserData: Observable<ProfileResponse> =
                getRequestsApi("https://steemit.com/", null).loadProfileExtendedData(nick)
...
//                    callback.onSuccessRequestFinish()
    ...
    fun saveData(
            su: CoinmarketcapCurrency,
            sdu: CoinmarketcapCurrency,
            pr: ProfileResponse,
            bv: Array<Double>
    ) {
        if (su.currencyName.isNotEmpty()) {
            RepositoryProvider.getPreferencesRepository()
                    .saveSteemCurrency(su)
        }
        ...
        if (pr.userExtended != null) {
            RepositoryProvider.getPreferencesRepository()
                    .saveUserExtendedData(pr.userExtended as UserExtended)
        }
        if (bv.size == 2) {
            RepositoryProvider.getPreferencesRepository()
                    .saveTotalVestingData(bv)
        }
    }

    private fun getBalanceVestsObservable(): Observable<Array<Double>> {
    ...
    override fun loadExtendedPostData(
            name: String,
            permlink: String
    ): Observable<FeedFullData> {
    ...
    override fun loadOutputAmount(
            requestData: AmountRequestData,
            callback: NetworkRepository.OnRequestFinishCallback<OutputAmount>
    ) {
    ...
    override fun loadOutputAmounts(
