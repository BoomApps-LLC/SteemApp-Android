...
class CurrenciesListAdapter(val context: Context) : RecyclerView.Adapter<CurrencyViewHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CurrencyViewHolder {
        return CurrencyViewHolder(LayoutInflater.from(context).inflate(R.layout.item_current_rate, null))
    }
    ...
    override fun onBindViewHolder(holder: CurrencyViewHolder, position: Int) {
        ...
        holder.currencyInput.text = "1 ".plus(data[position].inputCoinType.toUpperCase())
        holder.currencyOutput.text = "$".plus(data[position].outputAmount.toString())
    }
}